...
int Trie::startsWith(std::string prefix)
{
    ...
    {
        counter = wordCount(prefix_ptr); //count the number of nodes marked as the end of a word starting using 'prefix_ptr' as the root
    }
...
{
    //if the current node is marked as the end of a word, print the string in the array
    if(root->getIsWord())
    {
        for(int i = 0; i < pos; i++)
        {
    ...
    for(int i = 0; i < 26; i++) //loop through the children of the node
    {
        if(root->children_[i] != nullptr)
        {
            ...
            displayHelper(root->children_[i], word_array, pos + 1); //recursive call the function to continue building the word
        }
    }
}
...
int Trie::wordCount(Node *root)
{
    ...
    for(int i = 0; i < 26; i++)
    {
        if(root->children_[i] != nullptr)
        {
            total += wordCount(root->children_[i]); //recursively call the function
        }
    }
